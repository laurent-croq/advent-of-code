...
print("answer2 = %d" % sum(grid[w][z][y][x] for w,z,y,x in [ [w,z,y,x] for w in range(len(grid)) for z in range(len(grid[w])) for y in range(len(grid[w][z])) for x in range(len(grid[w][z][y])) ]))
